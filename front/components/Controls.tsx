...
function ControlButton({ label, variant = 'default', disabled = false, onClick, icon, badge, className = '' }: ControlButtonProps) {
  ...
  return (
    ...
    >
      <button
        className={`w-11 h-11 rounded-full flex items-center justify-center transition-all duration-200 disabled:opacity-40 disabled:cursor-not-allowed hover:scale-105 active:scale-95 relative ${getButtonStyles()}`}
        onClick={onClick}
        disabled={disabled}
      >
